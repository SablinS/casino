...
API_TOKEN = '7502290754:AAGH4j9e5KMttx6V11v6l9EKIptWZWgrIM0'
...
def polling_with_retry(bot, retries=5, interval=15):
    for i in range(retries):
        try:
            bot.polling(non_stop=True)
        except RequestException as e:
            print(f"Polling failed: {e}. Retrying in {interval} seconds...")
            time.sleep(interval)
        else:
            break
    else:
        print("Max retries exceeded. Bot stopped.")
